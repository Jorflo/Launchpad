...
async function init() {
    fingerprint = await getUserFingerpint()
    loginBrowser();
}
...
async function loginBrowser() {
    ...
    let response = await driver.findElement(
        By.css('pre')
    ).getText();
    ...
    for (let c in cookies)
        if (cookies[c].name == 'lp-token')
            lptoken = cookies[c].value
...
async function getUserFingerpint() {
    const externalLoggingFile = filepath + '\\LaunchPad.libs\\Logs\\ExternalLogging.log';
    try {
        fs.accessSync(externalLoggingFile, fs.F_OK)
        ...
        for await (const line of rl) {
            if (line.includes(' uf=')) {
                ...
                for await (const w of a) {
                    if (w.includes('uf')) {
                        return w.replace('uf=', '').replace(',', '')
                    }
                }
            }
        }
        console.log("Could not find fingerprint in file")
        return "FAKEFINGERPRINTHERE";
    } catch {
        console.log("Could not find ExternalLogging.log")
        return "FAKEFINGERPRINTHERE"
    }
}